...
//  XAP
//
//  Created by Zhang Yi on 10/12/2015.
...
extension UIImage {
    ...
    func scaled(toMaxDimension resolution:CGFloat) -> UIImage?{
        ...
        return resized(withContentMode: .scaleAspectFit, bounds: CGSize(width: resolution, height:resolution), interpolationQuality: .default)
    }
}
...
extension UIImage {
    ...
    */
     ...
     */
    func draw(text:String, atPoint point:CGPoint, withAttributes attrs:[String:AnyObject]? = nil) -> UIImage {
        // Check scale
        UIGraphicsBeginImageContextWithOptions(self.size, false, self.scale)
        draw(in: CGRect(x: 0, y: 0, width: self.size.width, height: self.size.height))
